...
async def load_graphs_background(kg_manager: KnowledgeGraphManager, config: Dict[str, Any]) -> None:
    ...
    try:
        ...
        kg_dir = config.get("knowledge_graphs", {}).get("prebuilt_directory", "./data/prebuilt_graphs")
...
async def initialize_services(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not config.get("startup", {}).get("lazy_load_graphs", True) and startup_mode in ["graphs", "all"]:
            kg_dir = config.get("knowledge_graphs", {}).get("prebuilt_directory", "./data/prebuilt_graphs")
        ...
        conversation_manager = ConversationManager(model_manager=model_manager, kg_manager=kg_manager, config=config)
        ...
        async def conversation_memory_pressure(memory_percent):
            ...
            logger.warning(f"Memory pressure detected ({memory_percent}%), optimizing conversation memory")
...
def init_app(app: FastAPI, config: Dict[str, Any]) -> None:
    ...
    async def startup_event():
        try:
            ...
            dynamic_storage = config.get("knowledge_graphs", {}).get("dynamic_storage", "./data/dynamic_graph")
    ...
    async def shutdown_event():
        try:
            ...
            if connection_manager:
                for session_id, connection in list(connection_manager.active_connections.items()):
                    try:
                        ...
                        # Ignore "websocket.close already sent" errors
                        if "websocket.close" in str(e):
                            logger.debug(f"WebSocket for session {session_id} already closed")
                        else:
                            logger.error(f"Error closing WebSocket connection for session {session_id}: {str(e)}")
            ...
            # Shutdown orchestrated model manager if used
            ...
            if conversation_manager and hasattr(conversation_manager, "sessions"):
                try:
                    ...
                    logger.info(f"Saved {session_count} conversation sessions")
                except Exception as e:
