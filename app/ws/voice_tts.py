...
async def tts_websocket(websocket: WebSocket, session_id: str, app):
    ...
    try:
        ...
        try:
            while True:
                ...
                try:
                    ...
                    logger.info(f"TTS request for session {session_id}: '{text[:50]}{'...' if len(text) > 50 else ''}'")
                    ...
                    chunk_count = 0
                    try:
                        async for audio_chunk in voice_manager.synthesize_response(session_id, text):
                            if websocket.application_state == WebSocketState.CONNECTED:
                                await websocket.send_bytes(audio_chunk)
                                chunk_count += 1
                            else:
                                logger.info(f"TTS WebSocket disconnected during synthesis for session {session_id}")
                                break
                        ...
                        logger.info(f"TTS completed for session {session_id}: {chunk_count} chunks sent")
                        
                    except Exception as e:
                        logger.error(f"Error during TTS synthesis for session {session_id}: {str(e)}")
                        if websocket.application_state == WebSocketState.CONNECTED:
                            await websocket.send_json({
                                "type": "tts_error",
        ...
        logger.info(f"TTS WebSocket connection duration: {time.time() - connection_start_time:.2f}s")
