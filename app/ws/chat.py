...
async def chat_websocket(websocket: WebSocket, session_id: str, app):
    ...
    client_info = {"ip": websocket.client.host, "session_id": session_id, "connection_time": datetime.now().isoformat()}
        ...
        try:
            while True:
                ...
                try:
                    ...
                    if websocket.application_state == WebSocketState.CONNECTED:
                        await ws_error(
                            websocket, "Communication error", extra={"details": str(e), "code": "WEBSOCKET_RECEIVE_ERROR"}
                        )
                    break
                ...
                try:
                    ...
                    if not user_message.strip():
                        ...
                        await ws_error(
                            websocket, "Empty message received", code=1003, extra={"code": "EMPTY_MESSAGE"}
                        )
                        continue
                        
                except Exception as e:
                    logger.warning(f"Error processing WebSocket message from session {session_id}: {str(e)}")
                    await ws_error(
                        websocket, f"Invalid message: {str(e)}", code=1003, extra={"code": "MESSAGE_ERROR"}
                    )
                    continue
                ...
                if websocket.application_state == WebSocketState.CONNECTED:
                    try:
                        await websocket.send_json({"type": "typing", "status": "start", "message_id": message_id})
                    except (WebSocketDisconnect, RuntimeError):
                        logger.info(f"WebSocket disconnected during typing start for session {session_id}")
                        break
                ...
                try:
                    ...
                    async def process_with_timeout():
                        ...
                        async for token in conversation_manager.converse_with_context(session_id, user_message):
                            response_text += token
                    ...
                    logger.info(f"Processed message {message_id} in {time.time() - process_start_time:.2f}s")
                    ...
                    if "session not found" in str(e).lower():
                        error_info["recovery"] = "Your session may have expired. Try refreshing the page."
                    elif "model" in str(e).lower() and "not available" in str(e).lower():
                        error_info["recovery"] = "The AI model is currently unavailable. Please try again later."
                ...
                if websocket.application_state == WebSocketState.CONNECTED:
                    try:
                        await websocket.send_json({"type": "typing", "status": "stop", "message_id": message_id})
                    except (WebSocketDisconnect, RuntimeError):
                        logger.info(f"WebSocket disconnected during typing stop for session {session_id}")
                        break
                ...
                if conversation_manager and websocket.application_state == WebSocketState.CONNECTED:
                    try:
                        ...
                        logger.info(f"📚 Retrieved citations data: {len(citations_data.get('citations', []))} citations")
                        ...
                        if citations_data.get("citations"):
                            ...
                            logger.info(f"✅ Sent {len(citations_data['citations'])} citations to WebSocket")
                        else:
                            await websocket.send_json(
                                {
                                    "type": "citations",
                                    "data": {"citations": [], "message": "No citations available for this response"},
                                    "message_id": message_id
                                }
                            )
                        ...
                        if dynamic_triples:
                            ...
                            logger.info(f"✅ Sent {len(dynamic_triples)} dynamic triples to WebSocket")
                            for triple in dynamic_triples:
                                logger.info(f"   📊 {triple.get('subject', 'N/A')} -> {triple.get('predicate', 'N/A')} -> {triple.get('object', 'N/A')}")
                        else:
                            logger.info("📭 No dynamic triples to send")
                            
                    except (WebSocketDisconnect, RuntimeError):
                        logger.info(f"WebSocket disconnected during citations send for session {session_id}")
                        break
                    except Exception as e:
            ...
            logger.info(f"WebSocket connection duration: {time.time() - connection_start_time:.2f}s")

        except Exception as e:
            logger.error(f"Error in WebSocket communication: {str(e)}")
            if websocket.application_state == WebSocketState.CONNECTED:
                await ws_error(
                    websocket, "WebSocket communication error", extra={"details": str(e), "code": "WEBSOCKET_ERROR"}
                )
            if connection_manager:
                connection_manager.disconnect(session_id)
            # Unregister from dynamic model manager
