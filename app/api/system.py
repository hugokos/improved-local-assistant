    ...
    try:
        ...
        try:
            model_status = await asyncio.wait_for(
                model_manager.get_model_status(), timeout=get_timeout("health_check_timeout", config)
            )
        ...
        active_connections_count = len(connection_manager.active_connections) if connection_manager else 0
        ...
        return {
            ...
            "timestamp": datetime.now().isoformat(),
            "uptime": system_monitor.metrics["system"]["uptime_seconds"] if system_monitor else None,
