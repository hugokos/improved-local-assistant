...
from app.schemas import GraphRequest, GraphCreateRequest, GraphTraversalRequest
...
async def create_graph(request: Request, graph_request: GraphCreateRequest):
    ...
    try:
        ...
        graph_id = kg_manager.create_graph_from_documents(graph_request.docs_path, graph_request.graph_id)
        if not graph_id:
...
async def graph_traversal(request: Request, traversal_request: GraphTraversalRequest):
    ...
    try:
        ...
        paths = kg_manager.get_graph_traversal(
            traversal_request.source, 
            traversal_request.target, 
            traversal_request.max_hops
        )
...
async def export_graph_interop(request: Request, graph_id: str, format: str = "networkx"):
    ...
    try:
        ...
        if not kg_manager:
            raise HTTPException(
                status_code=HTTP_STATUS["SERVICE_UNAVAILABLE"], detail="Knowledge graph manager not initialized"
            )
...
async def export_graph_native(
    ...
    try:
        ...
        if any([limit, hops, max_nodes]):
            ...
            zip_path = await asyncio.to_thread(kg_manager.export_native_partial, graph_id, query="", **subgraph_params)
        else:
            # Full export
...
async def import_graph_native(
    ...
    try:
        ...
        if merge_strategy not in ["union", "prefer_base", "prefer_incoming"]:
            raise HTTPException(
                status_code=400, detail="merge_strategy must be 'union', 'prefer_base', or 'prefer_incoming'"
            )
        ...
        try:
            ...
            result_graph_id = await asyncio.to_thread(
                kg_manager.import_native, temp_file.name, graph_id, graph_type, merge_strategy, replace
            )

            # Notify connected clients
            if connection_manager:
                await connection_manager.notify_graph_update(result_graph_id, f"imported_{graph_type}")
...
async def merge_graphs(
    ...
    try:
        ...
        raise HTTPException(
            status_code=HTTP_STATUS["NOT_IMPLEMENTED"], detail="Graph merge functionality not implemented yet"
        )
...
async def get_subgraph(
    ...
    try:
        ...
        return {"graph_id": graph_id, "subgraph": subgraph_data, "timestamp": datetime.now().isoformat()}
...
async def import_graph(
    ...
    try:
        ...
        raise HTTPException(status_code=501, detail="Graph import functionality not implemented yet")
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Error importing graph: {str(e)}")
