...
class KnowledgeGraphPersistence:
    ...
    def _load_from_persistence_with_encoding_fix(self, graph_path: str):
        ...
        try:
            ...
            self.logger.info(f"[DEBUG] Loaded graph at {graph_path}: docs={docs} nodes={nodes} edges={edges}")
        ...
        try:
            ...
            if graph_type == "property" and hasattr(self.dynamic_kg, 'property_graph_store'):
                try:
                    ...
                    self.logger.info(f"Created kg.json with {len(nodes)} nodes and {len(edges)} edges")
    ...
    def _should_persist_dynamic_graph(self) -> bool:
        ...
        return (
            self._dynamic_updates_count >= self._persist_update_threshold or
            time_elapsed >= self._persist_interval
        )
        ...
        try:
            ...
            if hasattr(kg_index, 'storage_context'):
                ...
                if hasattr(storage_ctx, 'graph_store') and hasattr(storage_ctx.graph_store, 'rel_map'):
                    stats["num_relationships"] = sum(len(v) for v in storage_ctx.graph_store.rel_map.values())
