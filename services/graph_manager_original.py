...
class KnowledgeGraphManager:
    ...
    def _configure_llama_index(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.info(f"Configured LlamaIndex to use local embeddings: {EMBED_MODEL_NAME}")
            except ImportError:
                self.logger.warning(
        ...
        try:
            ...
            for filename in os.listdir(graph_path):
                if filename.endswith('.json'):
                    ...
                    # Copy non-JSON files as-is
                    shutil.copy2(os.path.join(graph_path, filename), os.path.join(temp_dir, filename))
    ...
    def query_graphs(self, query: str, context: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            for graph_id, kg_index in self.kg_indices.items():
                qe = kg_index.as_query_engine(include_text=False, response_mode="tree_summarize")  # KG-only answers
                query_engines.append(qe)
                graph_sources.append(graph_id)

            # Add dynamic graph if available
            if self.dynamic_kg:
                dynamic_qe = self.dynamic_kg.as_query_engine(include_text=False, response_mode="tree_summarize")
            ...
            error_response = handle_error(e, context={"query": query}, error_code="KNOWLEDGE_GRAPH_QUERY_ERROR")
    ...
    async def export_native(self, graph_id: str) -> str:
        ...
        try:
            ...
            if graph_id == "dynamic" or not os.path.exists(os.path.join(self.prebuilt_directory, graph_id)):
            ...
            try:
                ...
                if graph_type == "modular":
                    ...
                    if self.dynamic_kg is None:
                        ...
                        self.logger.info(f"Merged dynamic graph import {graph_id} with existing dynamic graph")
    ...
    async def _merge_dynamic_graph(self, incoming_persist_dir: str, merge_strategy: str = "union"):
        ...
        try:
            ...
            # Merge the graphs
            merged_graph = self._merge_networkx_graphs(existing_graph, incoming_graph, merge_strategy)
    ...
    async def _merge_modular_graph(self, incoming_persist_dir: str, graph_id: str, merge_strategy: str = "union"):
        """
        ...
        try:
            ...
            self.logger.info(f"Successfully merged modular graph {graph_id} using strategy: {merge_strategy}")
    ...
    async def _persist_dynamic_graph(self):
        ...
        try:
            ...
            await asyncio.to_thread(self.dynamic_kg.storage_context.persist, persist_dir=persist_dir)
