...
class ConfigValidator:
    ...
    def __init__(self):
        ...
        self.schema = {
                ...
                "schema": {
                    "conversation": {
                        ...
                        "schema": {
                            ...
                            "context_window": {"type": "integer", "required": False, "default": 8000},
                    ...
                    "knowledge": {
                        ...
                        "schema": {
                            "name": {"type": "string", "required": True},
                            "context_window": {"type": "integer", "required": False, "default": 2048},
            ...
            "knowledge_graphs": {
                ...
                "required": True,
                "schema": {
                    "prebuilt_directory": {"type": "string", "required": False, "default": "./data/prebuilt_graphs"},
                    "dynamic_storage": {"type": "string", "required": False, "default": "./data/dynamic_graph"},
            ...
            "conversation": {
                ...
                "schema": {
                    ...
                    "context_window_tokens": {"type": "integer", "required": False, "default": 8000},
                },
            },
            "system": {
                "type": "dict",
                ...
                "schema": {
                    ...
                    "memory_threshold_percent": {"type": "number", "required": False, "default": 80},
                ...
                if not isinstance(env_config, dict):
                    errors.append(
                        ConfigValidationError(
                            path=env_key, message="Environment-specific configuration must be a dictionary"
                        )
                    )
        ...
        for key, field_schema in schema.items():
            ...
            if field_schema.get("required", False) and key not in config:
                ...
                if "default" in field_schema:
                    ...
                    errors.append(ConfigValidationError(path=field_path, message="Missing required field"))
            ...
            if key in config:
                ...
                if field_type == "dict":
                    if not isinstance(field_value, dict):
                        errors.append(
                            ConfigValidationError(
                                path=field_path, message=f"Expected dictionary, got {type(field_value).__name__}"
                            )
                        )
                    elif "schema" in field_schema:
                        # Recursively validate nested dictionary
                        self._validate_dict(field_value, field_schema["schema"], field_path, errors, warnings)

                elif field_type == "list":
                    if not isinstance(field_value, list):
                        errors.append(
                            ConfigValidationError(
                                path=field_path, message=f"Expected list, got {type(field_value).__name__}"
                            )
                        )
                    elif "schema" in field_schema:
                        ...
                        for i, item in enumerate(field_value):
                            ...
                            self._validate_value(item, field_schema["schema"], item_path, errors, warnings)

                elif field_type == "string":
                    if not isinstance(field_value, str):
                        errors.append(
                            ConfigValidationError(
                                path=field_path, message=f"Expected string, got {type(field_value).__name__}"
                            )
                        )
                    elif "allowed" in field_schema and field_value not in field_schema["allowed"]:
                        errors.append(
                            ConfigValidationError(
                                path=field_path, message=f"Value must be one of: {', '.join(field_schema['allowed'])}"
                            )
                        )
                    ...
                    if not isinstance(field_value, int):
                        errors.append(
                            ConfigValidationError(
                                path=field_path, message=f"Expected integer, got {type(field_value).__name__}"
                            )
                        )
                    elif "min" in field_schema and field_value < field_schema["min"]:
                        errors.append(
                            ConfigValidationError(
                                path=field_path, message=f"Value must be at least {field_schema['min']}"
                            )
                        )
                    elif "max" in field_schema and field_value > field_schema["max"]:
                        errors.append(
                            ConfigValidationError(
                                path=field_path, message=f"Value must be at most {field_schema['max']}"
                            )
                        )
                    ...
                    if not isinstance(field_value, (int, float)):
                        errors.append(
                            ConfigValidationError(
                                path=field_path, message=f"Expected number, got {type(field_value).__name__}"
                            )
                        )
                    elif "min" in field_schema and field_value < field_schema["min"]:
                        errors.append(
                    ...
                    if not isinstance(field_value, bool):
                        errors.append(
                            ConfigValidationError(
                                path=field_path, message=f"Expected boolean, got {type(field_value).__name__}"
                            )
                        )
    ...
    def _validate_value(
        ...
        if field_type == "dict":
            if not isinstance(value, dict):
                errors.append(
                    ConfigValidationError(path=path, message=f"Expected dictionary, got {type(value).__name__}")
                )
            ...
            if not isinstance(value, list):
                errors.append(ConfigValidationError(path=path, message=f"Expected list, got {type(value).__name__}"))
            elif "schema" in schema:
                # Validate each item in the list
            ...
            if not isinstance(value, str):
                errors.append(ConfigValidationError(path=path, message=f"Expected string, got {type(value).__name__}"))
            elif "allowed" in schema and value not in schema["allowed"]:
                errors.append(
                    ConfigValidationError(path=path, message=f"Value must be one of: {', '.join(schema['allowed'])}")
                )

        elif field_type == "integer":
            if not isinstance(value, int):
                errors.append(ConfigValidationError(path=path, message=f"Expected integer, got {type(value).__name__}"))
            elif "min" in schema and value < schema["min"]:
                errors.append(ConfigValidationError(path=path, message=f"Value must be at least {schema['min']}"))
            elif "max" in schema and value > schema["max"]:
                errors.append(ConfigValidationError(path=path, message=f"Value must be at most {schema['max']}"))

        elif field_type == "number":
            if not isinstance(value, (int, float)):
                errors.append(ConfigValidationError(path=path, message=f"Expected number, got {type(value).__name__}"))
            elif "min" in schema and value < schema["min"]:
                errors.append(ConfigValidationError(path=path, message=f"Value must be at least {schema['min']}"))
            elif "max" in schema and value > schema["max"]:
                errors.append(ConfigValidationError(path=path, message=f"Value must be at most {schema['max']}"))
            ...
            if not isinstance(value, bool):
                errors.append(ConfigValidationError(path=path, message=f"Expected boolean, got {type(value).__name__}"))

    def _check_unknown_fields(
        self, config: Dict[str, Any], schema: Dict[str, Any], path: str, warnings: List[ConfigValidationError]
    ) -> None:
        """
        ...
        for key in config:
            ...
            if key not in schema:
                warnings.append(ConfigValidationError(path=field_path, message="Unknown field", severity="warning"))
    ...
    def _apply_defaults_to_dict(self, config: Dict[str, Any], schema: Dict[str, Any]) -> Dict[str, Any]:
        """
