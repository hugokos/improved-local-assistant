...
class ConversationManager:
    ...
    async def process_message(self, session_id: str, user_message: str) -> AsyncGenerator[str, None]:
        ...
        session["messages"].append({"role": "user", "content": user_message, "timestamp": datetime.now()})
        ...
        if self.model_manager and self.kg_manager:
            ...
            self.logger.warning("Model manager or KG manager not available, using single-model mode")
        ...
        session["messages"].append({"role": "assistant", "content": assistant_response, "timestamp": datetime.now()})
        ...
        if self.metrics["messages_processed"] > 1:
            self.metrics["avg_response_time"] = (
                self.metrics["avg_response_time"] * (self.metrics["messages_processed"] - 1) + elapsed
            ) / self.metrics["messages_processed"]
        else:
            self.metrics["avg_response_time"] = elapsed

    async def converse_with_context(self, session_id: str, user_message: str) -> AsyncGenerator[str, None]:
        """
        ...
        from services.graceful_degradation import degradation_manager, with_degradation
        ...
        if session_id not in self.sessions:
            ...
            error_response = handle_error(error, context={"session_id": session_id}, error_code="SESSION_NOT_FOUND")
        ...
        try:
            ...
            if self.kg_manager:
                try:
                    ...
                    async def _kg_fallback():
                        self.logger.warning(f"Using fallback for knowledge graph query in session {session_id}")
                        return {
            ...
            has_system_prompt = any(msg.get("role") == "system" and "assistant" in msg.get("content", "").lower() for msg in messages)
...
"""
                ...
                self.logger.info(f"Added system prompt for conversation continuity in session {session_id}")
            ...
            if kg_results and "response" in kg_results and kg_results["response"]:
                ...
                self.logger.info(f"Added knowledge graph context to response for session {session_id}")
            ...
            if self.model_manager and self.kg_manager:
                try:
                    ...
                    def bg_task_error_handler(task):
                        if task.exception():
                            self.logger.error(f"Background knowledge graph update failed: {task.exception()}")

                    bg_task.add_done_callback(bg_task_error_handler)

                except Exception as e:
                    # Log but continue - background task failure shouldn't affect conversation
                    self.logger.error(f"Failed to start background knowledge graph update: {str(e)}")

                # Stream response from conversation model (Hermes 3:3B)
                # This ensures conversation flow remains uninterrupted
                self.logger.info(f"Streaming context-aware conversation response for session {session_id}")
            else:
                self.logger.warning("Model manager or KG manager not available, using single-model mode")
            ...
            # Get user-friendly error message
            error_response = handle_error(e, context={"session_id": session_id}, error_code="CONVERSATION_ERROR")

            yield f"I'm sorry, there was an issue processing your message. {error_response['suggestion']}"

    async def get_response(self, session_id: str, user_message: str, use_kg: bool = True) -> AsyncGenerator[str, None]:
        """
    ...
    async def _maybe_summarize_conversation(self, session_id: str) -> None:
        ...
        if session["summary"] and len(messages) - session["metadata"].get("last_summarized_at", 0) < 10:
            return
        ...
        try:
            ...
            summary_prompt = "Summarize this conversation concisely, capturing key points and context:\n\n"
    ...
    def manage_context_window(self, session_id: str) -> bool:
        ...
        if estimated_tokens > self.context_window_tokens * 0.8:
            ...
            if session.get("summary"):
                ...
                self.logger.info(f"Compressed history for session {session_id} using existing summary")
                return True
            else:
    ...
    def handle_topic_change(self, session_id: str, message: str) -> bool:
        ...
        has_topic_indicator = any(indicator in message.lower() for indicator in topic_change_indicators)
    ...
    def get_citations(self, session_id: str) -> Dict[str, Any]:
        ...
        for i, node in enumerate(source_nodes):
            ...
            if node_metadata:
                node_source = (
                    node_metadata.get("file_name") or 
                    node_metadata.get("source") or 
                    node_metadata.get("document_title") or
                    node_metadata.get("file_path", "").split("/")[-1] if node_metadata.get("file_path") else
                    "Unknown"
                )
            ...
            if success or True:  # Always create triples for demo purposes
                ...
                if self.kg_manager and hasattr(self.kg_manager, 'add_triples_to_dynamic_graph'):
                    try:
                        ...
                        self.logger.info(f"✅ Added {len(llamaindex_triples)} triples to persistent dynamic graph")
                ...
                if session_id in self.sessions:
                    ...
                    if len(self.sessions[session_id]["dynamic_triples"]) > 20:
                        self.sessions[session_id]["dynamic_triples"] = self.sessions[session_id]["dynamic_triples"][-20:]
                    
                    self.logger.info(f"🔗 Added {len(triples_created)} dynamic triples for session {session_id}")
                    for triple in triples_created:
                        self.logger.info(f"   {triple['subject']} -> {triple['predicate']} -> {triple['object']}")
    ...
    async def process_with_dual_model(self, session_id: str, user_message: str) -> AsyncGenerator[str, None]:
        """
