...
class VoskSTTService:
    ...
    def __init__(self, config: Dict = None):
        ...
        self.command_phrases = [
            "stop", "cancel", "mute", "repeat", "slower", "slow down", 
            "faster", "speed up", "normal speed", "reset speed",
            "new chat", "clear chat", "start over", "summarize", "summary",
            "cite sources", "show sources", "delete last", "undo"
        ]
    ...
    async def create_recognizer(self, session_id: str) -> bool:
        ...
        try:
            ...
            command_recognizer = vosk.KaldiRecognizer(self.model, self.sample_rate, self.command_grammar)
            ...
            self.logger.info(f"Created Vosk recognizers (dictation + command) for session: {session_id}")
            return True
    ...
    async def finalize_utterance(self, session_id: str) -> Dict:
        ...
        try:
            ...
            self.logger.info(f"🏁 Vosk Result() for {session_id}: '{text}' ({word_count} words, meaningful={is_meaningful})")
    ...
    def _is_meaningful_text(self, text: str, word_count: int) -> bool:
        ...
        valid_single_words = {
            'yes', 'no', 'okay', 'ok', 'hello', 'hi', 'help', 'stop', 'start', 
            'thanks', 'thank', 'please', 'sorry', 'what', 'why', 'how', 'when', 'where'
        }
        ...
        try:
            ...
            if len(audio_data) not in expected_frame_sizes:
                ...
                if hash(audio_data) % 50 == 0:  # Log ~2% of frames
                    self.logger.debug(f"⚠️ Non-standard frame size for {session_id}: {len(audio_data)} bytes (expected: {expected_frame_sizes})")
            ...
            if len(audio_data) > 0 and hash(audio_data) % 100 == 0:  # Log ~1% of chunks
                ...
                self.logger.debug(f"🎵 Server audio stats for {session_id}: bytes={len(audio_data)} absmax={absmax} rms={rms} samples={n} rate={self.sample_rate}")
            ...
            if dictation_recognizer.AcceptWaveform(audio_data):
                ...
                if command_recognizer:
                    ...
                    if cmd_partial_text and any(phrase in cmd_partial_text.lower() for phrase in self.command_phrases):
                        if cmd_partial_text != last_partial:
                            self._last_partials[session_id] = cmd_partial_text
                            self.logger.debug(f"Partial command for {session_id}: '{cmd_partial_text}'")
                            return {
                                "partial_command": cmd_partial_text
                            }
