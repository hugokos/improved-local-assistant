...
class KnowledgeGraphOptimizer:
    ...
    async def optimize_query(self, query: str) -> str:
        ...
        words = query.split()
        optimized_words = [word for word in words if word.lower() not in stop_words or len(words) <= 3]
    ...
    async def prune_dynamic_graph(self, max_nodes: int = 1000) -> bool:
        ...
        try:
            ...
            storage_ctx = self.kg_manager.dynamic_kg._storage_context.__class__.from_defaults(graph_store=graph_store)
    ...
    def save_cache_to_disk(self, cache_file: str = "data/kg_cache.json") -> bool:
        ...
        try:
            ...
            for key, value in self.query_cache.items():
                serializable_cache[key] = {"result": value["result"], "timestamp": value["timestamp"]}
