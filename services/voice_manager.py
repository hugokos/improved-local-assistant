...
class VoiceManager:
    ...
    async def on_vad_end(self, session_id: str) -> Dict:
        ...
        try:
            ...
            if not final_text and session.get("partial_transcript"):
                ...
                self.logger.info(f"Merged partial into empty final for {session_id}: '{final_text}'")
            ...
            self.logger.info(f"VAD end for {session_id}: '{final.get('final', '')}' (endpoint={final.get('endpoint', False)})")
            return final
    ...
    def _validate_vad_frame(self, frame_data: bytes) -> bool:
        ...
        if len(frame_data) not in valid_sizes:
            if len(frame_data) > 0:  # Only log non-empty invalid frames
                self.logger.debug(f"❌ Invalid VAD frame size: {len(frame_data)} bytes (expected: {valid_sizes})")
            return False
        
        # Additional validation: ensure even byte count (16-bit samples)
        if len(frame_data) % 2 != 0:
            self.logger.warning(f"❌ Odd frame size: {len(frame_data)} bytes (16-bit PCM requires even bytes)")
            return False
    ...
    def _update_vad_hysteresis(self, session_id: str, is_speech: bool, frame_data: bytes) -> Dict:
        ...
        if is_voiced:
            ...
            if not vad_state["is_active"] and vad_state["voiced_frames"] >= vad_state["frames_to_start"]:
                vad_state["is_active"] = True
                self.logger.info(f"🎤 VAD: Utterance STARTED for {session_id} (voiced_frames={vad_state['voiced_frames']}, rms={rms:.4f})")
            ...
            vad_state["voiced_frames"] = max(0, vad_state["voiced_frames"] - 1)  # Decay voiced frames
            ...
            if vad_state["is_active"] and vad_state["silence_frames"] >= vad_state["frames_to_end"]:
                ...
                if vad_state["silence_frames"] >= hangover_frames:
                    ...
                    self.logger.info(f"🔇 VAD: Utterance ENDED for {session_id} (hangover={vad_state['hangover_ms']}ms, rms={rms:.4f})")
    ...
    async def process_vad_frame(self, session_id: str, frame_data: bytes) -> Dict:
        ...
        try:
            ...
            if not self._should_process_audio(session_id):
                return {"skipped": "mic_muted_or_hold_off", "mode": self.half_duplex_sessions[session_id]["mode"]}
        ...
        try:
            ...
            async for audio_chunk in self.tts_service.synthesize_stream(text, session_id):
                ...
                if session.get("barge_in_requested", False):
                    self.logger.info(f"🚨 Barge-in detected for session {session_id}, stopping TTS after {chunk_count} chunks")
            ...
            if not session.get("barge_in_requested", False):
                self.logger.info(f"✅ TTS completed: {len(text)} chars, {chunk_count} chunks for session {session_id}")
            ...
            if self.metrics["tts_requests"] > 1:
                self.metrics["avg_tts_latency"] = (
                    self.metrics["avg_tts_latency"] * (self.metrics["tts_requests"] - 1) + total_latency
    ...
    async def handle_barge_in(self, session_id: str) -> bool:
        ...
        try:
            ...
            if self.tts_service and session.get("tts_active", False):
                ...
                session["tts_active"] = False
                self.logger.info(f"TTS cancellation for {session_id}: {'success' if cancel_success else 'failed'}")
