...
class SystemMonitor:
    ...
    def __init__(self, config=None):
        ...
        self.cpu_threshold = system_config.get("cpu_threshold_percent", get_threshold("cpu_warn", self.config))
        self.memory_threshold = system_config.get("memory_threshold_percent", get_threshold("mem_warn", self.config))
        ...
        self.metrics = {
            ...
            "performance": {"high_load_events": 0, "last_high_load": None, "adaptive_actions": 0, "last_cleanup": None},
            ...
            try:
                ...
                async with self.throttler:
                    ...
                    self.metrics["system"]["uptime_seconds"] = (
                        datetime.now() - datetime.fromisoformat(self.metrics["system"]["start_time"])
                    ).total_seconds()
    ...
    def _update_history(self, cpu_percent: float, memory_percent: float) -> None:
        ...
        if len(self.metrics["history"]["cpu"]) > max_history:
            ...
            self.metrics["history"]["timestamps"] = self.metrics["history"]["timestamps"][-max_history:]
    ...
    async def _check_high_load(self) -> None:
        ...
        if is_high_load:
            ...
            if current_time - self._startup_time < self._startup_grace_period:
                self.logger.debug(f"Ignoring high load during startup grace period: CPU {cpu_percent}%, Memory {memory_percent}%")
                return
            ...
            if not self.quiet_monitoring:
                ...
                self.logger.debug(f"High system load detected: CPU {cpu_percent}%, Memory {memory_percent}%")
