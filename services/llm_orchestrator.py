...
class LLMOrchestrator:
    ...
    async def process_turn(
        ...
        try:
            ...
            try:
                ...
                self.logger.error(f"Failed to acquire LLM semaphore within {self.semaphore_timeout}s")
    ...
    async def _stream_conversation(self, messages: List[Dict[str, str]]) -> AsyncGenerator[str, None]:
        """
        Stream response from conversation model (Hermes).
    ...
    async def _extract_knowledge_background(self, text: str, session_id: str) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if self.should_skip_extraction():
                self.logger.info(f"Skipping extraction for session {session_id} due to resource pressure")
    ...
    async def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_extraction": self._active_extraction_task is not None and not self._active_extraction_task.done(),
