...
class ExtractionPipeline:
    ...
    def _split_text_into_chunks(self, text: str, max_tokens: int = 128) -> List[str]:
        ...
        try:
            ...
            self.logger.warning(f"Error in token-based chunking: {e}, falling back to character-based")
    ...
    async def extract_bounded(
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(f"Extraction timed out after {effective_budget['max_time_seconds']}s")
            ...
            if triples:
                await self._persist_triples(triples, conversation_history[0].get('session_id') if conversation_history else None)
    ...
    def _prepare_input_window(
        ...
        if conversation_history and self.input_window_turns > 0:
            ...
            recent_history = conversation_history[-self.input_window_turns * 2:]  # User + assistant pairs
    ...
    def _apply_budget_constraints(self, custom_budget: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
...
Relationships:"""
    ...
    def _validate_triple(self, triple: Triple) -> bool:
        ...
        if (len(triple.subject) > 50 or 
            len(triple.predicate) > 30 or 
            len(triple.object) > 50):
            return False
