...
class ModelManager:
    ...
    async def _resolve_model_names(self) -> None:
        ...
        try:
            ...
            if self.conversation_model not in available_models:
                ...
                for model in available_models:
                    if model.startswith(base_name + ':'):
                        self.logger.info(f"Resolved conversation model: {self.conversation_model} -> {model}")
            ...
            if self.knowledge_model not in available_models:
                ...
                for model in available_models:
                    if model.startswith(base_name + ':'):
                        self.logger.info(f"Resolved knowledge model: {self.knowledge_model} -> {model}")
        ...
        try:
            ...
            if self.healthcheck_mode == "version":
                ...
                try:
                    ...
                    if self.conversation_model not in available_models:
                        self.logger.warning(f"Conversation model {self.conversation_model} not found in Ollama")
                    if self.knowledge_model not in available_models:
                        self.logger.warning(f"Knowledge model {self.knowledge_model} not found in Ollama")
                ...
                await self.chat_client.chat(
                    ...
                    options={"temperature": config.temperature, "num_predict": 1},  # Minimal response for testing
                )
    ...
    async def query_conversation_model(
        ...
        try:
            ...
            error_response = handle_error(e, context={"model": self.conversation_model}, error_code="MODEL_QUERY_ERROR")
    ...
    async def query_knowledge_model(
        ...
        try:
            ...
            async def _fallback_query():
                ...
                simple_prompt = f"Extract key entities (people, places, concepts) from this text: {text}"
                ...
                try:
                    ...
                    formatted_content = "\n".join([f"({entity}, mentioned_in, text)" for entity in entities])
            ...
            try:
                ...
                self.logger.warning(f"Circuit breaker open for knowledge model, using fallback: {str(e)}")
    ...
    async def _run_background_task(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"content": f"Error in background task: {str(e)}", "model": self.knowledge_model, "error": str(e)}
    ...
    async def run_multiple_background_tasks(self, texts: List[str], max_concurrent: int = 2) -> List[Dict[str, Any]]:
        """
        ...
        """
        self.logger.info(f"Starting {len(texts)} background tasks with max_concurrent={max_concurrent}")
        ...
        for i, result in enumerate(results):
            if isinstance(result, Exception):
                ...
                processed_results.append(
                    {"content": f"Error: {str(result)}", "model": self.knowledge_model, "error": str(result)}
                )
            else:
        ...
        while True:
            try:
                ...
                if high_load and not self._is_under_load:
                    ...
                    self._is_under_load = False
                    self.logger.info(f"System load normalized: CPU {cpu_percent}%, Memory {memory_percent}%")
