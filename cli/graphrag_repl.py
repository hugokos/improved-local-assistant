...
class GraphRAGREPL:
    ...
    def __init__(self, use_kg: bool = True, max_triple_per_chunk: int = 4):
        ...
        logger.info(
            f"GraphRAG REPL initialized with use_kg={use_kg}, max_triple_per_chunk={max_triple_per_chunk}"
        )
            ...
            try:
                if EMBEDDING_AVAILABLE:
                    self.embedding_model = HuggingFaceEmbedding(
                        model_name="BAAI/bge-small-en-v1.5", device="cpu", trust_remote_code=False
                    )
    ...
    async def initialize_kg_manager(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to load pre-built graphs (continuing in lazy-load mode): {str(e)}"
                )
                ...
                if graph_id in self.kg_manager.kg_indices:
                    ...
                    try:
                        ...
                        if hasattr(kg_index, "storage_context"):
                            ...
                            if hasattr(storage_ctx, "docstore") and hasattr(
                                storage_ctx.docstore, "docs"
                            ):
                            ...
                            if hasattr(storage_ctx, "graph_store") and hasattr(
                                ...
                                assert (
                                    num_relations > 10
                                ), f"Graph {graph_id} too small: {num_relations} relations"
                                ...
                                try:
                                    ...
                                    if hasattr(storage_ctx.vector_store, "_collection"):
                                        if hasattr(storage_ctx.vector_store._collection, "count"):
                                            vector_count = (
                                                storage_ctx.vector_store._collection.count()
                                            )
                                    ...
                                    if vector_count > 0:
                                        ...
                                        if hasattr(storage_ctx, "graph_store") and hasattr(
                                            ...
                                            num_relations = sum(
                                                len(v)
                                                for v in storage_ctx.graph_store.rel_map.values()
                                            )
                                            if vector_count == 1 and num_relations > 100:
                                                logger.warning(
                                                    f"Graph {graph_id}: Vector/node mismatch - {vector_count} vectors vs {num_relations} relations"
                                                )
                                                logger.warning(
                                                    "This suggests embeddings weren't properly generated during graph build"
                                                )
                                    else:
                                        logger.warning(
                                            f"Graph {graph_id}: No vectors found - semantic search may not work"
                                        )

                                except Exception as ve:
                                    logger.debug(
                                        f"Could not check vector count for {graph_id}: {ve}"
                                    )
...
Examples:
    ...
    repl = GraphRAGREPL(use_kg=not args.no_kg, max_triple_per_chunk=args.max_triple_per_chunk)
