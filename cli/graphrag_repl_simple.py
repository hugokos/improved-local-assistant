...
class SimpleGraphRAGREPL:
    ...
    def __init__(self, use_kg: bool = True, max_triple_per_chunk: int = 4, citation_detail: str = "detailed", prebuilt_dir: str = "./data/prebuilt_graphs"):
        """
        ...
        logger.info(f"Simple GraphRAG REPL initialized with use_kg={use_kg}, max_triple_per_chunk={max_triple_per_chunk}, citation_detail={citation_detail}")
    ...
    async def initialize_embedding_model(self) -> bool:
        ...
        try:
            if not EMBEDDING_AVAILABLE:
                logger.warning("HuggingFaceEmbedding not available, skipping embedding model initialization")
                return False
    ...
    async def initialize_kg_manager(self) -> bool:
        ...
        try:
            ...
            if loaded_graphs:
                logger.info(f"Successfully loaded {len(loaded_graphs)} prebuilt graphs: {loaded_graphs}")
            else:
    ...
    async def _print_compact_citations(self, citations, citations_data):
        ...
        for citation in citations:
            ...
            file_info = (
                ...
                metadata.get("file_path", "").split("/")[-1] if metadata.get("file_path") else None
            )
            if file_info:
    ...
    async def _print_detailed_citations(self, citations, citations_data):
        ...
        for i, citation in enumerate(citations, 1):
            ...
            if metadata:
                ...
                elif "source" in metadata and metadata["source"] in ["survivalist", "dynamic", "prebuilt_survivalist"]:
            ...
            if metadata:
                ...
                other_fields = [k for k in metadata.keys() 
                              if k not in file_fields + location_fields + time_fields + 
                              ['text_length', 'node_type', 'has_content'] and
                              isinstance(metadata[k], (str, int, float)) and 
                              len(str(metadata[k])) < 100]
        ...
        if metadata_summary.get("has_relationships"):
            print(f"  Contains Knowledge Graph Relationships: Yes")
