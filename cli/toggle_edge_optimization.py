...
def show_status(config):
    ...
    if enabled:
        ...
        print(
            f"    - Weights: Graph={weights.get('graph', 0.6)}, BM25={weights.get('bm25', 0.2)}, Vector={weights.get('vector', 0.2)}"
        )
        ...
        if server_env:
            ...
            print(
                f"    - OLLAMA_MAX_LOADED_MODELS: {server_env.get('OLLAMA_MAX_LOADED_MODELS', 1)}"
            )
...
def enable_edge_optimization(config):
    ...
    if "server_env" not in config:
        config["server_env"] = {"OLLAMA_NUM_PARALLEL": 1, "OLLAMA_MAX_LOADED_MODELS": 1}
...
Examples:
    ...
    parser.add_argument(
        "--status", action="store_true", help="Show current edge optimization status"
    )

    parser.add_argument("--enable", action="store_true", help="Enable edge optimization features")

    parser.add_argument("--disable", action="store_true", help="Disable edge optimization features")
