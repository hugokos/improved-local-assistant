...
def detect_and_fix_encoding(file_path, backup=True):
    ...
    try:
        ...
        logger.info(
            f"{file_path.name} - Detected encoding: {detected_encoding} (confidence: {confidence:.2f})"
        )

        # If already UTF-8 with high confidence, skip
        if (
            detected_encoding
            and detected_encoding.lower() in ["utf-8", "ascii"]
            and confidence > 0.9
        ):
        ...
        if detected_encoding:
            try:
                ...
                logger.warning(
                    f"Failed to decode with {detected_encoding}, trying with errors='replace'"
                )
                text_content = raw_data.decode(detected_encoding, errors="replace")
        else:
            logger.warning(
                f"Could not detect encoding for {file_path.name}, trying UTF-8 with errors='replace'"
            )
...
def main():
    ...
    parser.add_argument("--dir", type=str, help="Directory containing JSON files to fix")
    parser.add_argument(
        "--files", nargs="+", help="Specific files to fix (e.g., docstore.json index_store.json)"
    )
    parser.add_argument(
        "--no-backup", action="store_true", help="Skip creating backups of original files"
    )
    parser.add_argument(
        "--survivalist",
        action="store_true",
