...
class GraphRAGBenchmark:
    ...
    async def benchmark_single_query(self, query: str) -> Dict[str, float]:
        ...
        try:
            ...
            try:
                ...
                print(f"    Retrieved {len(retrieved_context) if retrieved_context else 0} context items")
            except Exception as e:
                ...
                try:
                    ...
                    if isinstance(retrieved_context, dict):
                        ...
                        if 'response' in retrieved_context:
                            ...
                            print(f"    Enhanced query with context from response")
                        ...
                        enhanced_query = f"Context: {context_text}\n\nQuestion: {query}"
                        print(f"    Enhanced query with string context")
                except Exception as e:
    ...
    async def benchmark_pipeline(self, num_runs: int = 3) -> Dict[str, Any]:
        ...
        """
        print(f"Starting GraphRAG Pipeline Benchmark")
        ...
        if all_results:
            overall_stats = {
                ...
                "retrieval_std": statistics.stdev([r["retrieval"] for r in all_results]) if len(all_results) > 1 else 0,
                "ttft_std": statistics.stdev([r["ttft"] for r in all_results]) if len(all_results) > 1 else 0,
                "total_std": statistics.stdev([r["ttlt"] for r in all_results]) if len(all_results) > 1 else 0,
            }
        else:
            overall_stats = {
                "total_queries": len(self.test_queries),
...
async def main():
    ...
    try:
        ...
        if stats.get("successful_runs", 0) > 0:
            print(f"Successful runs: {stats['successful_runs']}/{stats['total_queries'] * args.runs}")
            print(f"Average retrieval time: {stats['avg_retrieval_time']:.3f}s (±{stats['retrieval_std']:.3f})")
            ...
            print("\nPipeline Breakdown:")
            print(f"  Retrieval: {stats['avg_retrieval_time']:.3f}s ({stats['avg_retrieval_time']/stats['avg_total_time']*100:.1f}%)")
            print(f"  Prep: {stats['avg_prep_time']:.3f}s ({stats['avg_prep_time']/stats['avg_total_time']*100:.1f}%)")
            print(f"  Model TTFT: {stats['avg_ttft']:.3f}s ({stats['avg_ttft']/stats['avg_total_time']*100:.1f}%)")
            print(f"  Model Generation: {stats['avg_total_time']-stats['avg_ttft']:.3f}s ({(stats['avg_total_time']-stats['avg_ttft'])/stats['avg_total_time']*100:.1f}%)")
        else:
