...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('fire_query.log')
    ]
)
                        ...
                        for node in graph.nodes():
                            ...
                            if query_lower in node_str:
                                results.append({
                                    'type': 'node',
                                    'content': str(node),
                                    'data': graph.nodes[node] if hasattr(graph.nodes[node], '__dict__') else {}
                                })
...
def main():
    ...
    # Summary
    total_results = sum(
        len(results['vector_results']) + len(results['keyword_results']) + len(results['graph_results'])
    ...
    print(f"📝 Check fire_query.log for detailed logs")
