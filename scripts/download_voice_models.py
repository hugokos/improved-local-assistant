            ...
            for chunk in response.iter_content(chunk_size=8192):
                if chunk:
                    ...
                    if total_size > 0:
                        ...
                        print(f"\r  Progress: {percent:.1f}% ({downloaded // 1024 // 1024}MB)", end='', flush=True)
...
def download_vosk_model(model_name: str, models_dir: Path):
    ...
    extracted_dirs = [d for d in models_dir.iterdir() if d.is_dir() and d.name.startswith('vosk-model')]
    if extracted_dirs:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Download voice models for the Improved Local AI Assistant")
    parser.add_argument("--vosk", choices=list(VOSK_MODELS.keys()) + ["all"], 
                       help="Download Vosk STT model(s)")
    parser.add_argument("--piper", choices=list(PIPER_VOICES.keys()) + ["all"],
                       help="Download Piper TTS voice(s)")
    ...
    if args.all or args.vosk:
        vosk_models = ["small-en"] if args.all else ([args.vosk] if args.vosk != "all" else list(VOSK_MODELS.keys()))
    ...
    if args.all or args.piper:
        piper_voices = ["en_US-lessac-medium"] if args.all else ([args.piper] if args.piper != "all" else list(PIPER_VOICES.keys()))
