        ...
        if response.status_code == 200:
            ...
            for model in required_models:
                if any(model in name for name in model_names):
                    ...
                    logger.warning(
                        f"⚠️  Model {model} not found. Consider running: ollama pull {model}"
                    )
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            for i, line in enumerate(lines):
                ...
                logger.info(
                    f"🖥️  GPU {i}: {free}MB free / {total}MB total ({usage_percent:.1f}% used)"
                )

                if free < 2000:  # Less than 2GB free
                    logger.warning(
                        f"⚠️  GPU {i} has low memory ({free}MB free). This may cause CUDA out of memory errors."
                    )
...
def check_encoding_issues():
    ...
    if encoding_issues:
        logger.info(
            "💡 Encoding issues detected. The system has fallback mechanisms to handle these."
        )
...
def check_system_resources():
    ...
    try:
        ...
        logger.info(
            f"💾 System Memory: {memory_available_gb:.1f}GB available / {memory_gb:.1f}GB total ({memory_percent:.1f}% used)"
        )
        ...
        logger.info(
            f"💿 Disk Space: {disk_free_gb:.1f}GB free / {disk_total_gb:.1f}GB total ({disk_percent:.1f}% used)"
        )
