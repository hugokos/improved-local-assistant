...
async def build_registry():
    ...
    try:
        ...
        embedder = HuggingFaceEmbedding(
            model_name="BAAI/bge-small-en-v1.5", device="cpu", normalize=True, embed_batch_size=10
        )
                    ...
                    try:
                        ...
                        if "nodes" in kg_data:
                            ...
                            if entity_names:
                                ...
                                embedding_text = (
                                    f"{graph_name} contains: {', '.join(entity_names[:10])}"
                                )
                                print(
                                    f"  Using property graph entities for embedding: {len(entity_names)} entities"
                                )
                            else:
