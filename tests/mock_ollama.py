...
class MockAsyncClient:
    ...
    async def chat(self, model: str, messages: List[Dict[str, str]], stream: bool = False, **kwargs) -> Any:
        ...
        response_content = f"Mock response to: {user_message[:50]}..." if user_message else "Mock chat response"
    ...
    async def pull(self, model: str, stream: bool = False) -> Any:
        ...
        if stream:
            async def mock_pull_stream() -> AsyncGenerator[Dict[str, Any], None]:
                statuses = ["pulling manifest", "downloading", "verifying sha256", "writing manifest", "success"]
