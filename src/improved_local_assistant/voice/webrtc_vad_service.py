...
class WebRTCVADService:
    ...
    def __init__(self, config: dict = None):
        ...
        self.aggressiveness = self.config.get(
            "aggressiveness", 2
        )  # START AT 2, NOT 3 (too aggressive clips speech)
        self.frame_duration_ms = self.config.get(
            "frame_duration_ms", 20
        )  # 20ms is sweet spot for most speech

        # Validate frame duration
        if self.frame_duration_ms not in [10, 20, 30]:
            raise ValueError(
                f"Frame duration must be 10, 20, or 30ms, got {self.frame_duration_ms}ms"
            )
        ...
        self.speech_threshold = self.config.get(
            "speech_threshold", 3
        )  # 3 voiced frames to start (60ms at 20ms frames)
        self.silence_threshold = self.config.get(
            "silence_threshold", 15
        )  # 15 silence frames to end (300ms hangover)

        self.logger.info(
            f"🎙️ WebRTC VAD initialized: {self.sample_rate}Hz, {self.frame_duration_ms}ms frames, "
    ...
    def process_audio(self, audio_data: bytes) -> list[tuple[bool, bytes]]:
        ...
        if not self.validate_audio_format(audio_data):
            self.logger.warning("Invalid audio format, skipping VAD processing")
            return results
        ...
        while len(self.frame_buffer) >= self.frame_bytes:
            ...
            if len(frame_data) != self.frame_bytes:
                self.logger.warning(
                    f"❌ WebRTC VAD frame size mismatch: {len(frame_data)} bytes, expected {self.frame_bytes}"
                )
                continue
            ...
            if sample_count != expected_samples:
                self.logger.warning(
                    f"❌ WebRTC VAD sample count mismatch: {sample_count} samples, expected {expected_samples}"
                )
                continue
            ...
            try:
                ...
                if len(results) % 50 == 0:  # Log every 50th frame (~1.5 seconds at 30ms frames)
                    self.logger.debug(
                        f"🎙️ WebRTC VAD: speech={is_speech}, smoothed_active={self.is_speech_active}, "
                ...
                self.logger.error(
                    f"Frame details: size={len(frame_data)}, expected={self.frame_bytes}, "
