...
class DynamicModelManager:
    ...
    def __init__(self, config: dict[str, Any], connection_pool: ConnectionPoolManager):
        ...
        self.current_conversation_model = (
            config.get("models", {}).get("conversation", {}).get("name", "hermes3:3b")
        )
        self.current_knowledge_model = (
            config.get("models", {}).get("knowledge", {}).get("name", "tinyllama")
        )

        # Available model options
        self.conversation_options = (
            config.get("models", {}).get("conversation", {}).get("options", [])
        )
        ...
        self.logger.info("Initialized DynamicModelManager")
        ...
        self.logger.info(
            f"Available conversation models: {[opt['name'] for opt in self.conversation_options]}"
        )
        self.logger.info(
            f"Available knowledge models: {[opt['name'] for opt in self.knowledge_options]}"
        )
    ...
    async def switch_conversation_model(self, model_name: str) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"Switched conversation model from '{old_model}' to '{model_name}' in {switch_time:.3f}s"
            )
    ...
    async def switch_knowledge_model(self, model_name: str) -> dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"Switched knowledge model from '{old_model}' to '{model_name}' in {switch_time:.3f}s"
            )
    ...
    def _get_model_config(self, model_type: str, model_name: str) -> dict[str, Any] | None:
        ...
        """
        options = (
            self.conversation_options if model_type == "conversation" else self.knowledge_options
        )
    ...
    async def _unload_model(self, model_name: str) -> None:
        ...
        try:
            ...
            self.logger.debug(
                f"Error unloading model '{model_name}' (may already be unloaded): {e}"
            )

    async def _notify_model_switch(
        self, model_type: str, old_model: str, new_model: str, switch_time: float
    ) -> None:
        """
