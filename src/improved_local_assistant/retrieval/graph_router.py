...
class GraphRouter:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            try:
                self.fast_llm = Ollama(model="hermes3:3b", request_timeout=30.0, temperature=0.0)
    ...
    async def _load_registry(self) -> bool:
        ...
        try:
            ...
            for name in self.graph_names:
                ...
                # Store metadata
                self.graph_meta[name] = {k: v for k, v in graph_data.items() if k != "vector"}
...
"""
    ...
    def _load_indices(self, graph_id: str):
        ...
        try:
            ...
            storage_ctx = StorageContext.from_defaults(
                persist_dir=str(base_dir), fs=get_utf8_filesystem()
            )
            ...
            if not vector_idx and not property_idx and len(indices) == 1:
                ...
                if hasattr(single_idx, "property_graph_store") or hasattr(
                    single_idx, "graph_store"
                ):
                    property_idx = single_idx
                    self.logger.debug("Single index identified as graph index")
                else:
                    vector_idx = single_idx
                    self.logger.debug("Single index identified as VectorStoreIndex")
            ...
            self.logger.info(
                f"Successfully loaded indices for {graph_id}: "
    ...
    def make_hybrid_retriever(
        self, vector_index=None, property_index=None, similarity_top_k: int = 25
    ):
        """
        ...
        try:
            ...
            if property_index:
                property_retriever = property_index.as_retriever(
    ...
    def make_auto_vector_retriever(self, index, similarity_top_k: int = 50):
        ...
        try:
            ...
            auto_retriever = AutoVectorRetriever(vector_retriever=base, llm=self.fast_llm)
    ...
    async def generate_response(self, query: str, context_nodes: list[Any]) -> str:
        ...
        try:
            ...
            if (
                hasattr(context_nodes[0], "score")
                and context_nodes[0].score < self.min_score_threshold
            ):
            ...
            if len(sources) < 2:
                return (
                    "I don't have enough data from multiple sources to provide a reliable answer."
                )
        ...
        try:
            ...
            if not top_graphs:
                return {
                    ...
                    "metadata": {"graphs_searched": [], "nodes_retrieved": 0, "query_time": 0},
                }
