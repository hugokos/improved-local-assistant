...
class ConnectionPoolManager:
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            try:
                ...
                self._client = httpx.AsyncClient(
                    base_url=self.ollama_host, limits=limits, timeout=timeout, http2=False
                )
    ...
    async def get_loaded_models(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Network-related errors during shutdown are expected
            self.logger.debug(f"Network error getting loaded models (likely during shutdown): {e}")
            return []
        except Exception as e:
    ...
    async def _periodic_residency_check(self) -> None:
        ...
        while True:
            try:
                ...
                # Skip check if client is not available (during shutdown)
                if not self._client:
                    break

                loaded_models = await self.get_loaded_models()
                if not loaded_models:  # Skip if we can't get model info
                    continue
                ...
                # Check if expected models are loaded (only if we have models to check)
                if model_names and self.keep_alive_models:
                ...
                self.logger.debug("Model residency check cancelled")
                break
            except Exception as e:
    ...
    async def shutdown(self) -> None:
        ...
        try:
            ...
            # Cancel residency check task with timeout
            if self._residency_check_task and not self._residency_check_task.done():
                self._residency_check_task.cancel()
                try:
                    await asyncio.wait_for(self._residency_check_task, timeout=2.0)
                except (asyncio.CancelledError, asyncio.TimeoutError):
                    self.logger.debug("Residency check task cancelled")
                except Exception as e:
                    self.logger.debug(f"Error cancelling residency check task: {e}")

            # Close HTTP client with error handling
            if self._client:
                try:
                    await asyncio.wait_for(self._client.aclose(), timeout=5.0)
                except asyncio.TimeoutError:
                    self.logger.warning("HTTP client close timed out")
                except Exception as e:
