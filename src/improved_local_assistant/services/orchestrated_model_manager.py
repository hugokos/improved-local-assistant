    ...
    def __init__(self, config: dict[str, Any], system_monitor: Optional[SystemMonitor] = None):
        ...
        try:
            ...
            self.extraction_pipeline = ExtractionPipeline(
                config, self.orchestrator.connection_pool, self.system_monitor
            )
        ...
        self.knowledge_model = (
            config.get("models", {}).get("knowledge", {}).get("name", "phi3:mini")
        )
    ...
    def _setup_llamaindex_llms(self):
        ...
        try:
            ...
            self.chat_llm = Ollama(
                model=self.conversation_model, base_url=self.host, request_timeout=120.0
            )

            self.kg_llm = Ollama(
                model=self.knowledge_model, base_url=self.host, request_timeout=60.0
            )
            ...
            try:
                ...
                Settings.embed_model = OllamaEmbedding(
                    model_name="nomic-embed-text", base_url=self.host, request_timeout=60.0
                )

            self.logger.info(
                f"✅ LlamaIndex global LLM set to: {type(Settings.llm).__name__} ({self.conversation_model})"
            )
            self.logger.info(
                f"✅ LlamaIndex global embeddings set to: {type(Settings.embed_model).__name__}"
            )
    ...
    async def initialize_models(self, config: ModelConfig) -> bool:
        ...
        try:
            ...
            if hasattr(self.system_monitor, "start_monitoring"):
                if (
                    not hasattr(self.system_monitor, "monitoring_task")
                    or self.system_monitor.monitoring_task is None
                ):
    ...
    async def query_conversation_model(
        ...
        try:
            ...
            async for token in self.orchestrator.process_turn(
                session_id=session_id, user_message=user_message, conversation_history=messages
            ):
                yield token
