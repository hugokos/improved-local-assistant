...
class HybridEnsembleRetriever(BaseRetriever):
    ...
    def __init__(
        ...
        *,
        ...
        self.graph = _make_graph_retriever(graph_index, depth=graph_depth, top_k=graph_top_k)
        self.vector = (
            vector_index.as_retriever(similarity_top_k=vector_top_k) if vector_index else None
        )
        self.bm25 = bm25_index.as_retriever(similarity_top_k=bm25_top_k) if bm25_index else None
        ...
        self.logger.info(
            f"Initialized HybridEnsembleRetriever with {len(self.live_retrievers)} active retrievers"
        )
    ...
    def _retrieve(self, query_bundle: QueryBundle) -> List[NodeWithScore]:
        ...
        try:
            ...
            if len(ranked_nodes) > self.max_chunks:
                ...
                self.logger.debug(
                    f"Budget exceeded: {len(ranked_nodes)} nodes reduced to {self.max_chunks}"
                )
    ...
    def _apply_colbert_rerank(self, nodes: List[NodeWithScore], query: str) -> List[NodeWithScore]:
        ...
        try:
            ...
            reranker = ColbertRerank(model="colbert-ir/colbertv2.0", top_n=self.rerank_top_n)
            ...
            if document_nodes and BM25Retriever:
                try:
                    ...
                    if hasattr(document_nodes[0], "node_id"):
                        ...
                        bm25_nodes = [
                            TextNode(text=doc.text, metadata=doc.metadata) for doc in document_nodes
                        ]
    ...
    async def retrieve_chunks(
        ...
        try:
            ...
            if session_id and self.working_set_cache:
                nodes_with_scores = await self._apply_working_set_boost(
                    nodes_with_scores, session_id
                )
