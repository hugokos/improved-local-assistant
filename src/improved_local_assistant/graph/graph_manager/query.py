    ...
    async def _legacy_query_graphs(
        ...
        try:
            ...
            if optimizer is not None:
                ...
                self.logger.debug(
                    "Knowledge graph optimizer not initialized, skipping optimization"
                )
                ...
                if nodes:
                    ...
                    class MockResult:
                        def __init__(self, nodes):
                            self.response = self._synthesize_response_from_nodes(
                                nodes, optimized_query
                            )
                    ...
                    for i, qe in enumerate(query_engines):
                        try:
                            ...
                            if source_nodes and len(source_nodes) > 0:
                                self.logger.info(
                                    f"Got {len(source_nodes)} results from {graph_sources[i]}"
                                )
                                best_result = result
