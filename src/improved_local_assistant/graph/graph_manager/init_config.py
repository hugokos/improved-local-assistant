...
class KnowledgeGraphInitializer:
    ...
    def __init__(self, model_manager=None, config=None):
        ...
        self._entity_catalog_path = os.path.join(
            self.dynamic_storage, "main", "entity_catalog.json"
        )
        self._entity_catalog = (
            {}
        )  # {"person::hugo kostelni": {"id": "...", "embed": [..], "name": "Hugo Kostelni"}}
        ...
        try:
            ...
            if os.path.exists(meta_path):
                with open(meta_path, encoding="utf-8") as f:
                    ...
                    self.logger.info(
                        f"Found embedding model in survivalist metadata: {embed_model}"
                    )
                    return embed_model
                    ...
                    if os.path.exists(meta_path):
                        try:
                            with open(meta_path, encoding="utf-8") as f:
                                ...
                                embed_model = metadata.get("embed_model", default_model)
                                self.logger.info(
                                    f"Found embedding model in {graph_dir} metadata: {embed_model}"
                                )
                                return embed_model
                        except Exception as e:
    ...
    async def _initialize_graph_router(self):
        ...
        try:
            ...
            if not os.path.exists(registry_path):
                self.logger.info(
                    "Graph registry not found, skipping advanced router initialization"
                )
                return
